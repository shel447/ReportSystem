...
- **沉浸式生成交互**: 系统默认以对话助手启动，支持沉浸式聊天体验。
- **二维表格视图**: 实例列表通过专业二维表格展示 ID、状态、生成时间等核心信息。
- **模板匹配与参数收集**: 智能兜底逻辑确保对话过程中始终弹出参数配置表单。
- **实例操作**: 支持查看详情、删除实例以及局部重新生成（LLM Mock）。

### 2.3 报告文档管理 (已增强)

- **表格中心化展示**: 导出的 Word/PDF 记录统一在表格中展示大纲和路径。
- **文档操作**: 支持文件下载模拟与导出记录删除。
...
| v0.3 | 2026-03-04 | Antigravity | 同步实现：沉浸式对话 UI、表格化列表、实例管理 API 及 .gitignore 配置 |
