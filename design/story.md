...
## 场景八：从确认大纲处 fork 出一个新的对话分支（未来能力展望）

> 本场景描述的是**尚未实现**、但已经具备明确业务价值的一项未来能力：从某个历史对话记录的“大纲确认”阶段 fork 出一个新的对话分支，在继承原有参数和大纲的基础上，继续做差异化修改，再生成另一份报告。
...
陆Sir 不想重新再开一个对话，从头补参数、再改一遍大纲。他希望在当前这个大纲确认节点上，直接点一个动作：
...
- 继承已确认参数
- 继承当前已编辑的大纲树
- 记录 fork 来源会话与 fork 来源节点

于是，原始分支继续保留为“运维详细版”，而新分支则变成“管理摘要版”。

### 8.1 在 fork 分支里只改差异，不重复劳动
...
### 8.2 fork 之后，模板实例也应该分叉留痕

在这个未来能力里，陆Sir 还期待系统把 fork 行为也记录到模板实例链路里：

- 原模板实例：记录原始大纲确认状态
- fork 后模板实例：记录“来源于哪个模板实例快照”
- 新分支继续保存自己的 `outline_saved / outline_confirmed`
...
*本版用户故事已按当前系统实现更新，并额外补充了“从确认大纲节点 fork 新对话分支”的未来能力展望，覆盖系统设置、报告模板、模板实例、对话助手、报告实例与 Markdown 文档的最新交互链路。*
