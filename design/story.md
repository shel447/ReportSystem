...
> 陆Sir 是省公司网络运维与分析团队的负责人。以前，他每天要在模板整理、参数确认、数据核对、报告撰写、文档导出之间来回切换。系统不断演进后，他终于把这些动作串成了一条清晰、可追踪、可复用的工作流：**系统设置 -> 报告模板 -> 对话生成 -> 报告实例 -> Markdown 文档**，并且在同一个对话入口里顺手完成智能问数和智能故障分析。
...
其中“分析目标”被他设置成 `interaction_mode=chat`。这意味着在对话收参时，这个参数不会弹出结构化表单，而是由助手用自然语言继续追问。
...
## 场景十：同一入口里，报告与问数/故障可以切换，但不会乱

某天下午，陆Sir 正在制作“设备巡检日报”。系统已经帮他确认了：
...
系统没有把这句话误判成“智能问数”，而是优先把它当成当前参数答案继续收集。这让 `chat` 模式参数终于变得顺滑，不会因为一句自然语言就把报告流程打断。
...
如果他确认，系统就进入问数；如果取消，系统继续留在当前报告任务里。

陆Sir 对这种边界很认可：

> “回答一个参数，不应该误触发能力切换；但我明确说要换任务时，系统也不能装听不懂。”

---

## 场景十一：从已有报告实例创建定时任务，并把计划时间写入报告时间

月底例会前，陆Sir 需要一份每天早上自动生成的“总部巡检简报”。他不想再从头配置，而是直接打开一份已经生成好的报告实例，确认内容结构没问题后，进入 **定时任务** 页面点击“新建任务”。

这时系统不是要求他手填模板 ID，而是只让他选：
...
于是系统在后续定时执行时，会保留两套时间：

- `created_at`：真实执行时间
- `report_time`：业务上的报告时间

同时，若配置了时间参数名，系统还会把计划执行时间格式化后写回报告参数。

陆Sir 很看重这一点，因为他不希望日报的展示时间和实际跑任务的机器时间混成一回事。

他对团队说：

> “一份定时报表既要知道它是什么时候跑出来的，也要知道它代表的是哪一天、哪一个时点的业务口径。这两个时间不能混。”

---

**— 故事结束 —**

*本版用户故事已按当前系统实现更新，并补充了统一对话模块、参数追问模式、定时任务双时间模型，以及“从历史消息或报告实例继续分支”的现态能力与后续演进方向，覆盖系统设置、报告模板、对话助手、报告实例与 Markdown 文档的最新交互链路。*
