...
> 说明：本文件是业务故事，不是规范定义。正式能力边界与接口契约统一以 [design/report_system/README.md](report_system/README.md) 为准。

> 陆Sir 是省公司网络运维与分析团队的负责人。以前，他每天要在模板整理、参数确认、数据核对、报告撰写、文档导出之间来回切换。系统重构后，他终于把这些动作串成了一条清晰、可追踪、可复用的工作流：**系统设置 -> 报告模板 -> 对话生成 -> 报告实例 -> 文档导出**。
...
陆Sir 把文档发给同事审阅，同事提出几点修改意见。陆Sir 没有重来一遍，而是回到报告实例页，基于返回的 `templateInstance` 重新进入对话链路，对目标诉求调整后生成新报告。
...
*本版用户故事已按当前公开实现更新，覆盖系统设置、报告模板、对话助手、报告实例与 Markdown 文档的完整链路，并补充了“从历史消息或报告实例继续分支”的现态能力与后续演进方向。*
