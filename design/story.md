...
1. **从对话历史中的某条消息 fork**
2. **从模板实例列表中的 `outline_saved` 快照 fork**
...
如果他是在“模板实例”页面里看见一条 `outline_saved` 记录，也可以直接点：

> Fork 到对话助手

系统会自动创建新会话，并直接打开对话助手，把他送回到 `大纲确认` 阶段：
...
- 从历史消息 fork 新会话
- 从 `outline_saved` 模板实例 fork 到对话助手
- 在新分支中保留来源标识
- 继续原有参数确认/大纲确认/生成链路
...
- 会话分支树可视化
- fork 后的会话重命名
- 模板实例链路里显示 fork 来源与差异
- 从更多阶段发起 fork，而不仅是 `outline_saved`
...
*本版用户故事已按当前系统实现更新，并补充了“从历史消息或模板实例 fork 新对话分支”的现态能力与后续演进方向，覆盖系统设置、报告模板、模板实例、对话助手、报告实例与 Markdown 文档的最新交互链路。*
