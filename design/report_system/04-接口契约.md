...
- 模板实例中的展开来源统一通过 `dynamicContext` 返回；`foreach/foreachCase` 字段包含 `type/parameterId/itemValue/caseId`，`custom` 字段包含 `type/url/nodeType`；旧 `foreachContext` 只作为历史数据读取兼容，不再作为新响应字段
- `dynamic.type = foreachCase` 时，服务端按 `ParameterValue.value` 匹配 `cases[].values`，多选参数按每个选中值生成对应分支内容

### Dynamic Custom 外部内容请求

`dynamic.type = custom` 在 Report DSL 生成阶段由服务端向模板定义的 `url` 发起 `POST` 请求。请求体最小结构如下：

```json
{
  "nodeType": "section",
  "nodeId": "section_external_recommendation",
  "parameters": {
    "scope": [
      {
        "label": "总部网络",
  ...
  "prompt": "基于总部网络在2026-04-18的运行数据生成外部分析建议。"
}
```

- `nodeType = catalog` 时，响应必须是完整 Report DSL `Catalog` 片段。
- `nodeType = section` 时，响应必须是完整 Report DSL `Section` 片段。
- 目录级请求的 `prompt` 使用目录实例 `renderedTitle`；章节级请求的 `prompt` 使用编辑后的 `outline.renderedRequirement`。
...
- 普通 `table` block 中的 `properties.columns/showTitle/defaultDisplayRows/mergeColumns` 必须按原结构返回；`showTitle` 只控制标题展示，`defaultDisplayRows` 只表示默认展示条数，不截断 dataset 结果
- `composite_table.parts[]` 中的 `tableLayout.columns/showTitle/defaultDisplayRows/mergeColumns` 必须按原结构返回；`columns[].key` 使用源数据列 key，`columns[].title` 是展示列名
...
- `report.reportMeta[sectionId].outline` 必须返回章节大纲骨架与实例化结果
