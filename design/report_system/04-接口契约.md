...
- `generate_report_segment` 场景下，`delta` 复用 `add_section` 动作；前端根据 `sectionId` 是否已存在于当前报告中区分"新增"与"替换"语义
- `add_section` 在 `generate_report_segment` 中的最小字段与 `generate_report` 一致：`sectionId`、`status`、`requirement`，如已有组件可附带 `components`
