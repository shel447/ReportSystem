...
- Report DSL 表格行合并结果使用 `dataProperties.mergeRows[]`，每项包含 `startRowIndex/rowSpan/mergedText/column`；不得使用旧字段名 `columnKey`
...
- `report.structureType = paged` 时，主体使用 `content`，且不得同时返回 `catalogs`
...
- `report.reportMeta[sectionId].additionalInfo` 使用新版 `{type, content}` 数组字段
