...
- 目录和章节的动态展开统一采用 `dynamic` 字段；`type` 支持 `foreach/foreachCase/custom`
- `foreachCase` 是特殊 foreach，不是单选 switch；它按参数每个取值匹配 case，并使用 case 中不同的 `subCatalogs/sections` 定义生成内容
- `custom` 使用显式 `url` 字段，不再保留占位 `config`；章节级 custom 仍必须提供 `outline` 以支持用户编辑大纲
...
- 普通 `table` block 通过 `properties` 承载展示属性；当前包含 `columns/showTitle/defaultDisplayRows/mergeColumns`
- `composite_table.parts[].tableLayout` 继续承载复合表子表布局，包含 `columns/showTitle/defaultDisplayRows/mergeColumns`
