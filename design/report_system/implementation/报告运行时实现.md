...
- flow 输出必须显式写入 `structureType = flow`，并输出新版 `catalogs[].title`，不再输出旧 `catalogs[].name`
...
- 章节摘要进入 `reportMeta[sectionId].summary`，不再输出旧 `sections[].summary`
...
- custom 目录响应按新版 `ReportCatalog` 解析并替换当前目录，响应必须包含 `title`
- custom 章节响应按新版 `ReportSection` 解析并替换当前章节，响应不得包含旧 `summary` 字段
