...
- `outline.renderedRequirement` 用 `display` 语义生成，可将多值按 `、` 连接
...
|---|---|---|
| `parameters`、`catalog.parameters`、`section.parameters` | 以统一参数模型收集不同作用域的报告输入、候选项、实际取值和确认态 | 通过内容生成结果间接体现，并沉淀到 `reportMeta` |
| `catalogs`、`catalog.subCatalogs` | 决定正式目录层级 | `catalogs`、`subCatalogs` |
| `section.outline.requirement + section.outline.items` | 以统一诉求模型承载章节诉求骨架与实例化结果 | `reportMeta[sectionId].question` |
...
| `sections[].outline.renderedRequirement` | 写入 `reportMeta[sectionId].question` | 作为章节生成时的正式诉求文本 |
| `章节生成结果状态` | 写入 `reportMeta[sectionId].status` | 必须使用 `report.schema.json` 中的 `Running/Success/Aborted/Failed` |
| `sections[].outline.items[].values` | 间接用于生成组件数据、摘要和 `reportMeta.additionalInfos` | 不直接写入 DSL 顶层固定字段 |
| `sections[].executionBindings[].resolvedQuery` | 写入 `reportMeta[sectionId].additionalInfos` 中的 `SQL` 类信息 | 仅保留冻结后的执行证据 |
| `parameters` | 不直接进入 DSL 固定字段 | 仅通过生成结果和 `reportMeta` 间接体现 |
...
- `Report DSL` 中不得出现 `deltaViews`、`templateSkeletonStatus`、实例态 `parameters`、实例态 `outline` 这类运行态对象
