...
|---|---|---|
| `text` | `text` | 模板态 `template` 在实例化阶段渲染参数引用与 dataset 字段引用为实例态 `content`，编译时进入 `dataProperties.content` |
| `table` | `table` | 直接映射 |
| `chart` | `chart` | 直接映射 |
...
- `text` block 编译为 DSL `TextComponent`，其中实例态 `content -> dataProperties.content`
- `text.template` 中 `{$parameterId}` 引用参数，`{#datasetId.field}` 引用同一 section 内 dataset 执行结果第一行的字段值；一个 text block 可以引用多个 dataset 字段
