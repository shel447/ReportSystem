...
| `custom` | 不在模板实例化阶段展开；Report DSL 冻结阶段通过外部 URL 获取目录或章节 DSL 片段 |
...
- `dynamic.type = custom` 的目录或章节实例输出 `dynamicContext.type = custom`，并记录 `url/nodeType`
...
`dynamic.custom` 的冻结规则：

- 目录级 custom 使用当前目录实例的 `renderedTitle` 作为请求体 `prompt`，把父级和当前目录可见参数放入 `parameters`，POST 到 `dynamic.url`，响应按 Report DSL `Catalog` 片段解析并替换当前目录输出。
- 章节级 custom 使用用户可编辑后的 `outline.renderedRequirement` 作为请求体 `prompt`，把父级和当前章节可见参数放入 `parameters`，POST 到 `dynamic.url`，响应按 Report DSL `Section` 片段解析并替换当前章节输出。
...
| `section.content.datasets` | 决定数据获取与加工 | 组件数据、`reportMeta[sectionId].additionalInfo` |
...
| `sections[].runtimeContext.bindings[].resolvedQuery` | 写入 `reportMeta[sectionId].additionalInfo` 中的 `SQL` 类信息 | 仅保留冻结后的执行证据 |
...
- `mergeRows` 只在 `TableComponent.dataProperties.data` 已存在时计算；运行时按 `data` 行顺序扫描 `column` 对应字段，连续相同值且跨度大于 1 时生成 DSL `{startRowIndex,rowSpan,mergedText,column}`
...
### 3.5 `reportMeta.additionalInfo` 类型约定

`reportMeta[sectionId].additionalInfo[*].type` 必须使用 DSL 既有枚举，不得自造新值。每一项固定使用 `{type, content}`，不再输出旧字段 `value`。首版映射建议固定如下：

| 来源 | `additionalInfo[*].type` |
|---|---|
...
| `basicInfo.title` | 典型取报告日期、统计周期或对象范围 |
| `basicInfo.description` | 模板描述或实例化后的报告描述 |
| `basicInfo.createdAt/updatedAt` | 报告实例创建/更新时间 |
...
- 章节摘要进入 `reportMeta[sectionId].summary`
- 顶层 `summary.content`：基于章节总结再聚合生成
- 若尚未完成汇总，生成中状态可暂不填顶层 `summary`

`cover` 建议来源：

- 首选：模板约定的封面策略或系统默认封面模板
- 新版 `cover` 为组件容器，字段固定为 `components[]`
...
- 默认不强制生成
- 仅当模板或业务场景明确要求签署页时生成
- 新版 `signaturePage` 为组件容器，字段固定为 `components[]`
