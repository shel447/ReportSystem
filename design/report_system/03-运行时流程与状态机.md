...
`reportMeta[sectionId].additionalInfo[*].type` 必须使用 DSL 既有枚举，不得自造新值。每一项至少包含 `type`，可按证据形态补充 `name/value/content/appendix`；实现层兼容读取 `additionalInfos`，但新输出优先使用 `additionalInfo`。首版映射建议固定如下：
...
| `basicInfo.subTitle` | 可取关键对象范围、汇报场景或副标题 |
