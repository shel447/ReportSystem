...
- 普通 `table` block 编译为 DSL `TableComponent`，其中 `datasetId -> dataProperties.sourceId`；展示属性由 `properties` 承载，当前定义包含 `columns/showTitle/defaultDisplayRows/mergeColumns`
...
- `query part`
  ...
  - `tableLayout` 仅作为该子表的布局约束，不改变其“普通表格”语义；其中 `columns/showTitle/defaultDisplayRows/mergeColumns` 与普通表格同名展示属性语义一致
