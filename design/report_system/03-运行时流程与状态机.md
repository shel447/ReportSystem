# 03. 运行时流程与状态机

## 1. 总体阶段

统一对话中的报告生成分为四个阶段：

1. 模板识别与参数收集
2. 模板实例构建与诉求确认
3. `TemplateInstance -> Report DSL`
4. 报告冻结与文档生成

## 2. TemplateInstance 的形成过程

### 2.1 参数收集

- 参数值统一写入 `TemplateInstance.parameterValues`
- 参数值统一保存为三元组数组
- 动态参数候选项通过 `openSource.url` 解析

### 2.2 诉求实例化

系统使用模板中的：

- `parameters`
- `catalogs`
- `section.requirement`
- `section.content`

生成运行态 `TemplateInstance`：

- `catalogs`
- `section.requirementInstance`
- `section.executionBindings`
- `templateSkeletonStatus`

### 2.3 前台修改与骨架状态

前台可修改完整 `TemplateInstance` 树，并可附带平铺 `deltaViews`。

后台处理规则：

- 只改槽位值：`templateSkeletonStatus.internal` 保持 `reusable`
- 改坏结构化诉求骨架：降级到 `conditionally_reusable` 或 `broken`
- UI 只看到 `not_broken / broken`

## 3. TemplateInstance -> Report DSL

### 3.1 应用层职责

`TemplateInstance -> Report DSL` 是应用层正式能力，不是基础设施适配工作。

应用层需要完成：

1. 读取模板实例树状主体
2. 解释执行绑定与数据集定义
3. 为每个 section 生成内容组件
4. 组装正式 `Report DSL`
5. 用 `report-dsl.schema.json` 校验结果
6. 将校验通过的 DSL 冻结到 `ReportInstance`

### 3.2 模板各组成部分的应用方式

| 模板组成 | 运行时作用 | 在 Report DSL 中的落点 |
|---|---|---|
| `parameters` | 收集报告输入、驱动数据查询和文案生成 | 通过内容生成结果间接体现，并沉淀到 `reportMeta` |
| `catalogs` | 决定正式目录层级 | `catalogs` |
| `section.requirement` | 决定章节诉求和生成问题 | `reportMeta[sectionId].question` |
| `section.content.datasets` | 决定数据获取与加工 | 组件数据、`reportMeta.additionalInfos` |
| `section.content.presentation` | 决定段落、表格、图表、markdown 等组件布局 | `section.components` |

## 4. LLM 交互点

### 4.1 模板匹配

输入：用户问题、会话上下文、模板摘要

输出：候选模板或无模板结论

提示词骨架：

```text
你是报告模板匹配器。
任务：根据用户问题，从候选模板中选择最合适的一份。
要求：只根据模板用途、参数要求、目录结构进行判断；若都不适合，返回 none。
输出：模板 id、置信度、理由。
```

### 4.2 诉求补全与确认文案

输入：模板定义、已收集参数、用户补充表达

输出：`TemplateInstance.requirementInstance`

提示词骨架：

```text
你是报告诉求整理器。
任务：把用户输入和模板槽位合并成结构化诉求实例。
要求：保留目录层级，不新增模板不存在的章节；槽位值要转换成 display/value/query 三元组语义。
输出：catalog -> section -> requirementInstance。
```

### 4.3 报告内容生成

输入：章节诉求、数据结果、模板呈现约束

输出：章节组件内容

提示词骨架：

```text
你是报告内容生成器。
任务：基于章节诉求与数据结果生成正式报告组件内容。
要求：只输出目标 section 对应的组件；需要摘要时同时给出 section summary；禁止改动目录结构。
输出：components、summary、reportMeta 补充信息。
```

## 5. 状态机

### 5.1 对话轮状态

- `waiting_user`
- `running`
- `finished`
- `failed`

### 5.2 模板实例状态

- `draft`
- `collecting_parameters`
- `ready_for_confirmation`
- `confirmed`
- `generating`
- `completed`
- `failed`

### 5.3 报告状态

- `generating`
- `available`
- `failed`

### 5.4 文档生成状态

- `queued`
- `running`
- `ready`
- `failed`

## 6. 时序总览

```mermaid
sequenceDiagram
    participant U as User
    participant Chat as Conversation API
    participant App as Report Runtime App
    participant Ext as Option Source
    participant Exp as Document Generator

    U->>Chat: POST /chat
    Chat->>App: build / update TemplateInstance
    App->>Ext: resolve dynamic options (optional)
    App-->>Chat: ask(fill_params or confirm_params)
    U->>Chat: POST /chat reply=confirm_params
    Chat->>App: freeze TemplateInstance and generate Report DSL
    App-->>Chat: stream REPORT answer
    U->>Chat: POST /reports/{reportId}/document-generations
    Chat->>Exp: generate word/ppt
    Exp-->>Chat: artifacts
    Chat-->>U: documents ready
```
