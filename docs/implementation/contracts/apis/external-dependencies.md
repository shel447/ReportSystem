...
实体详情中的 `schema.fields[]` 完整进入 NL2SQL Prompt。`string/boolean/int/long/float/double/decimal/date/datetime/timestamp` 可映射为 Ibis 字段；`array/record/object` 仅作为元数据提供给 Prompt，首版禁止参与可执行查询。数据集和逻辑关系对象继续作为开放 JSON object 消费。
