...
|---|---|---|---|
| 查询逻辑实体列表 | `POST /rest/odae/v3/datacatalog/model/logicalentities/list` | `pageSize/pageNo/filter.includeSchemaOfParent` | 候选摘要 `retCode/retInfo/data.results[].name` 及平台扩展字段 |
| 查询逻辑实体 | `GET /rest/odae/v3/datacatalog/model/logicalentity` | `logicalEntityName` | 完整 `data`，遵循 LogicalEntity 契约 |
| 查询数据集 | `GET /rest/odae/v3/datacatalog/model/datasets/{name}` | 路径参数 `name` | `data` |
| 查询逻辑关系列表 | `POST /rest/dte/v2/datacatalog/product/model/logicalrelations/query` | `pageSize/pageNo/filter` | 关系名称摘要 `retCode/retInfo/data.results[].name` |
| 查询逻辑关系 | `GET /rest/dte/v2/datacatalog/product/model/logicalrelation` | `name` | 完整 `data`，遵循 LogicalRelationship 契约 |
...
逻辑实体列表只用于识别候选实体，不是完整字段 Schema 的事实源。NL2SQL 选出候选后，必须逐个调用单实体详情接口；即使列表响应携带类似 Schema 的扩展字段，也不能替代详情请求。选中实体详情会按用户和实体名称缓存，并严格按照 [logical-entity.schema.json](../schemas/logical-entity.schema.json) 校验；缺失或非法详情会终止本次 NL2SQL。未被选中的摘要不影响本次生成。

实体详情中的 `schema.fields[]` 完整进入 NL2SQL Prompt。`string/boolean/int/long/float/double/decimal/date/datetime/timestamp` 可映射为 Ibis 字段；`array/record/object` 仅作为元数据提供给 Prompt，首版禁止参与可执行查询。

关系列表同样只用于获取名称。NL2SQL 逐个请求并按 [logical-relationship.schema.json](../schemas/logical-relationship.schema.json) 严格校验完整详情，任一非法详情都会终止本次生成；随后只保留连接已选实体的关系。完整关系进入 Prompt，只有 `Source.field = Target.field` 形式、且实体与基础字段均有效的单字段等值条件会进入 Ibis Join 白名单。数据集对象继续作为开放 JSON object 消费。
