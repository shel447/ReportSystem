...
Knowledge 不可用时降级为空上下文；DataCatalog、SQL 安全检查或 OneQuery 失败时停止本次智能问数。DataCatalog 实体列表只负责候选识别，NL2SQL 对每个选中实体逐个请求并严格校验完整详情；关系列表只提供名称，系统逐个校验关系详情后筛选已选实体关系。复杂字段和非单字段等值关系保留在 Prompt，但不进入可执行 Ibis 能力白名单。
...
指标 key 使用实际请求详情并参与 NL2SQL 的逻辑实体名称。列举候选摘要不计数。AgentFlow 不理解 DataCatalog 或逻辑实体含义，只在终态 metrics 的 `uniqueCounts` 中透传该 metric name 的去重数量。
