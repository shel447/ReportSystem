...
- `modules/backend` 从 FastAPI/uvicorn 切换为 Tornado；业务接口、开发辅助接口、静态页面、文件下载、SSE 和 OpenAPI 均由 `RequestHandler` 承载。
- `RequestHandler` 严格停留在 Web 适配层；Handler 删除 `Depends(get_db)`、`Session` 和 ORM 依赖。
...
- `shared/kernel` 新增 `@policy_auth(resource, action)` 路由注解、权限元数据和通用 FastAPI dependency。
- `/rest/chatbi/v1/*` 正式业务路由统一在进入业务逻辑前调用 Policy Authentication，开发辅助接口和前端静态资源不纳入该鉴权。
- 平台鉴权实现位于 `infrastructure/platform`，使用 `GET /rest/dte/smartbi/v1/proxy/auth/chat` 并透传当前请求头；无权限统一转换为 `chatbi.base.permission.denied`。
...
- `GET /chat/{conversationId}` 公开响应改为 `records[]`，每条 record 表示一轮对话，并包含 `chatId/question/askTime/answers[]`；不再返回旧 `messages[]`。
...
- 公开停止接口收口为 `POST /chat/{chatId}/stop`；内部 `runId` 不再出现在 ChatResponse、SSE、前端类型或 API 契约中。
