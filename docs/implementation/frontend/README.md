...
- 模板列表采用高密度工作台样式，提供搜索、结构类型筛选、分类筛选、紧凑列式信息和右侧操作入口。
- 模板详情采用左侧结构导航 + 右侧编辑面板，不再把元信息、参数、结构和危险操作平铺成松散卡片。
- 参数编辑使用紧凑行 + 详情展开形式，默认只展示参数摘要，候选项和默认值等低频 JSON 内容按需展开编辑。
- flow 目录和章节编辑使用可折叠树节点，根目录默认展开，子目录、section 和低频配置默认收起，优先展示路径、标题、动态展开、section 数量和核心诉求，减少多层卡片嵌套。
- paged 模板继续以结构摘要和 JSON 查看为主，但使用同一工作台导航和紧凑列表表达章节、页面与 section。
