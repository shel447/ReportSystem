...
| 外部查询字段 | Report DSL `Column` | 备注 |
|---|---|---|
| `columns[field]` 的 key | `key` | 保留结果行字段名 |
| `columns[field].type` | `type` | 进入 DSL 前执行类型归一化，例如 `number -> double` |
| `lineageTracing.sources[0].businessName_cn/businessName` | `title` | 按中文业务名、业务名、字段 key 顺序回退 |
| `lineageTracing.sources[]` | `lineageTracing.sources[]` | 保留全部来源；查询层专属 `type/dataSourceType` 不进入 DSL |

- 报告生成固定传入 `context["lineage.tracing.enable"] = true`，因此每个成功返回字段必须带至少一个来源
- `retCode != 0` 的查询业务失败归一化为空数据，告警进入日志和 `TemplateInstance.warnings`
- HTTP 错误、超时或不满足 Schema 的查询响应继续作为生成失败处理
