# 报告生成实现

字段级结构以 [Report DSL Schema](../contracts/schemas/report-dsl.schema.json) 和 [Report DSL 技术手册](../contracts/manuals/报告DSL定义与使用说明书.md) 为准。历史上从业务规格中迁出的详细运行规则保留在 [业务规则技术参考](business-rules.md)。

## 1. 模块定位

`report` context 中的报告生成源文件负责两个正式能力：

- `TemplateInstance -> Report DSL`
- `Report DSL -> DocumentArtifact`

## 2. 领域对象

- `TemplateInstance`
- `ReportContext`
- `ReportDsl`
- `ReportInstance`
- `DocumentArtifact`
- `ExportJob`

`ReportInstance` 是资源包络；`content` 保存完整 `Report DSL`。

实现约束补充：

- `TemplateInstance` 和 `ReportDsl` 必须是递归 dataclass 聚合，不能只在顶层定义 dataclass、内部仍用 `dict/list[dict]`
- `TemplateInstance`、`ReportDsl` 及其子对象内部属性名保持 `snake_case`，对外公开字段名统一依赖 `field(metadata={"alias": "lowerCamelCase"})`
- `TemplateInstance.template` 必须组合完整 `ReportTemplate` dataclass 快照
- `TemplateInstance.structureType` 缺省为 `flow`；flow 实例使用 `catalogs`，paged 实例使用 `chapters -> slides -> sections`
- `ReportDsl.structureType` 缺省为 `flow`；flow DSL 使用 `catalogs + layout`，paged DSL 使用 `content`
- 本轮补齐 paged Report DSL 核心模型与 schema；`ReportDslCompiler` 仍只编译 flow 结构，模板 paged 到 PPT DSL 的映射后续实现
- `ReportInstance.report` 必须组合完整 `ReportDsl` dataclass
- 入库、出库、导出时才允许做 JSON 序列化

## 3. 应用服务职责

当前代码中，`ReportService` 是 report context 的统一应用入口。内部协作服务包括 `ReportScenarioService`、`ReportParameterService`、`ReportTemplateService`、`ReportGenerationService` 和 `ReportDocumentService`。

### 3.1 `ReportService`

`ReportService` 是 router、场景 provider 和其他 context 访问 report context 的唯一入口：

- report 自己的场景 handler 通过 `ReportService.chat()` 交付严格 `ReportScenarioCommand`
- 模板、参数候选、报告详情和文档路由也只依赖 `ReportService`
- 内部服务可以协作，但不直接泄漏给 router 或其他 context

### 3.2 `ReportScenarioService`

`ReportScenarioService` 是报告业务接入通用对话的场景编排器：

- 处理 `generate_report`、`extract_report_template` 与 `generate_report_segment` instruction。
- 识别模板，并调用 `ReportParameterService` 处理参数语义。
- 接收外部系统首次交接的 `report.templateName + report.parameters`，精确定位模板并初始化根级参数快照。
- 推进同一个 `TemplateInstance`，并在确认后调用报告冻结服务。

报告场景统一通过 `shared/agentflow` 表达为 Flow。报告流程通过 `FlowContext.emit_step()` 发送阶段进展，通过 `emit_delta()` 发送报告增量，通过 `emit_answer()` 返回最终 `REPORT`、`REPORT_SEGMENT` 或模板预览 answer。step 应尽量提供 `parentStepId/stepPath`，delta 应提供统一 `parent`，同时保留 flow 的 `parentCatalogId/parentCatalog` 和 paged 的 `chapterId/slideId` 兼容字段。后续细化节点时可逐步使用 tool、prompt、hook、checkpoint、子流程和动态追加分支；首版不要求把现有业务推进拆成过细节点。

可复用的多步骤能力应优先封装为 AgentFlow 子流程，例如章节生成、数据分析和复杂内容评估。子流程内部的 step/delta/error 通过 `sourceSubflow` 与 `source` 汇入父流程；子流程 answer 默认只写入父流程 state 或作为 `subflow_result` delta，不覆盖报告流程最终 `REPORT`，除非显式启用 answer 冒泡。

`ReportContext` 是报告场景运行上下文，当前包含完整 `TemplateInstance`。其中来源归因统一使用 `conversationId/chatId`；它不管理聊天消息状态。

### 3.3 `ReportParameterService`

承担报告参数相关的完整应用职责：

- 提取用户自然语言中已出现的参数值
- 解析 `reply.parameters`
- 调用动态候选值数据源
- 校验外部系统交接的根级参数定义，并将外部 `options/defaultValue/values` 合并到模板副本
- 仅对外部尚未赋值的参数继续执行自然语言补提取
- 判断必填参数缺失
- 构造报告场景 `ask.parameters` 与 `ask.reportContext`

领域层的 `ParameterResolver` 负责纯参数归一化、作用域解析、标量转换和缺参判断，不访问外部数据源，不构造聊天响应。动态候选值的本地或 HTTP 调用由基础设施 gateway 负责。

外部交接载荷由 report context 自己提供的场景 codec 解码为严格 `ReportBootstrapRequest`。conversation 只透传顶层 JSON，不识别模板或参数。首版只允许交接 `ReportTemplate.parameters` 根级参数；catalog、section、chapter 与 slide 局部参数继续由 ReportSystem 自身流程解析。

### 3.4 `ReportGenerationService`

承担以下功能职责：

- `persist_template_instance`
  - 新建或更新 `TemplateInstance`
  - 保证对话层始终围绕同一份模板实例推进
- `get_latest_template_instance`
  - 按会话读取最近一次模板实例快照
- `generate_report_from_template_instance`
  - 读取模板实例与模板快照
  - 解析 custom 动态内容
  - 调用纯领域 `ReportDslCompiler` 构建 `Report DSL`
  - 通过 Schema gateway 校验 DSL
  - 创建 `ReportInstance`
  - 把模板实例状态推进到 `completed/report_ready`
- `get_report_view`
  - 读取报告核心视图；文档列表由 `ReportService` 组合
- `serialize_report_answer`
  - 统一 `/chat` 与 `/reports` 的报告答案载荷
- `preview_section_regeneration`
  - 加载 `ReportInstance` 与关联 `TemplateInstance`
  - 在 `TemplateInstance.catalogs` 树中定位目标 `TemplateInstanceSection`
  - 应用请求中的新 `outline`，标记 `user_edited = true`，评估 `skeleton_status`
  - 基于新 `outline.items` 与 `section.content.datasets` 重新调用 `build_execution_bindings()`
  - 调用 `_build_section_components(section)` 生成新 components、summary、additional_infos
  - 返回 `ReportSection` DSL 片段与 `ReportGenerateMeta`，不持久化任何对象

### 3.5 `ReportDocumentService`

承担以下功能职责：

- 把下载能力收束为 report-scoped 子服务
- 校验文档格式和复用策略
- 生成 Word/PPT/Markdown 产物；PDF 暂未开放
- 创建 `ExportJob` 和 `DocumentArtifact`
- 提供报告文档列表和 report-scoped 下载
- 不暴露独立 document 资源集合

## 4. 领域职责

- `ParameterResolver`：参数归一化、作用域收集和值映射
- `ReportDslCompiler`：把已解析的实例树编译为 flow Report DSL；不访问数据库、HTTP 或 Schema 文件
- `template_instance_builder`：构建 `TemplateInstance`，并在同一递归过程中展开 `foreach/foreachCase/custom` 动态结构
- `placeholder_renderer`：渲染参数占位符和诉求要素占位符

application 层的 custom 内容解析器负责调用外部 gateway 并校验返回片段，再把强类型 catalog/section 交给 compiler。

## 5. ReportDslCompiler 规则

输入：

- 完整 `TemplateInstance`
- 模板快照
- 数据查询结果
- LLM 生成结果

输出：

- 满足 `docs/implementation/contracts/schemas/report-dsl.schema.json` 的合法 `Report DSL`

必须完成：

1. 读取 `structureType` 对应的实例树；flow 读取 `catalogs -> (subCatalogs)* -> sections`，paged 读取 `chapters -> slides -> sections`
2. 保留 `TemplateInstance.section.content` 的实例化视图
3. 根据 `runtimeContext.bindings` 生成查询表达式
4. 解析 `section.content.datasets`
5. 将 `presentation.blocks` 编译为 `components`
6. 填充新版 `basicInfo / layout / reportMeta / summary`
7. 校验 DSL

补充规则：

- flow 输出必须显式写入 `structureType = flow`，但 `catalogs` 分支保持 PPT 扩展前的旧契约：`Catalog.name`、`Section.summary`、`Cover.title` 等字段不因 paged 能力改变
- paged 输出使用 `content`，内容元素只能是 `ReportSlide[]` 或 `ReportSlideSection[]`，不得混放
- `reportMeta` 仍按全局唯一 `sectionId` 挂载
- `reportMeta[sectionId]` 对齐 BI Engine `GenerateMeta`：公开输出只包含 `status/question/additionalInfos/outline/parameters`
- `reportMeta[sectionId].additionalInfos` 使用复数字段名，条目必须包含 `type/value`，可包含 `name/appendix`；模型层兼容读取旧 `additionalInfo` 与 `content`
- `reportMeta[sectionId].parameters` 使用完整模板参数 `Parameter` 结构，避免和模板/实例态参数定义分叉
- `reportMeta[sectionId].outline` 的 schema 定义名为 `GenerateOutline`，后端复用 `OutlineDefinition + RequirementItem` 序列化，并保留 `isBroken` 标记
- `basicInfo` 模型需支持 BI Engine 资产字段，包括 `schemaVersion/mode/status/name/reportType/description/templateId/templateName/remark/version/createDate/modifyDate/creator/modifier/header/footer/category`；`title/parameters/createdAt/updatedAt` 不作为正式 Report DSL 字段输出
- `cover.layoutTemplate` 严格输出 `TITLE_TOP | TITLE_CENTER`，不再输出旧示例值 `default`
- `backCover` 作为 paged/PPT 封底配置挂在 `ReportDsl.back_cover`，公开字段为 `backCover`
- `layout.autoLayout`、表格列 `sortable/filterable/width/type/enumConfig/uiConfig/lineageTracing`、`mergeColumns[].isMergeValue`、图表 `series` 与组件 `basicProperties/advanceProperties` 均作为 DSL 序列化边界透传字段；图表响应式、X 轴标签模式和 SQL 解释位于 `ChartAdvanceProperties`，不再使用 `ChartComponent.options`
- 表格行合并模型统一为 `MergeRowInfo`，公开字段仍为 `startRowIndex/rowSpan/mergedText/column`
- 图表轴配置输出到 `ChartDataProperties.x_axis/y_axis`，公开字段为 `dataProperties.xAxis/yAxis`；旧顶层输入只做读取兼容
- 章节摘要继续输出到 `sections[].summary`，如需作为生成证据则写入 `reportMeta[sectionId].additionalInfos[]` 的 `Summary` 项
- 目录树位置由正式 `catalogs/subCatalogs/sections` 结构表达
- 生成进度主指标按 section 统计，目录级上下文通过 `generationProgress.currentCatalogPath` 辅助表达
- 参数与诉求要素的占位符渲染默认读取 `label` 通道；显式指定时才读取 `.value / .query`
- 模板实例化阶段必须解释 `dynamic`：
  - `foreach` 保持同一模板按参数值重复展开
  - `foreachCase` 按每个参数值匹配 case，并生成该 case 中定义的目录或章节内容
  - `custom` 不在实例化阶段展开，只在目录、章节或分页页面实例上保留 `dynamicContext.type/url/nodeType`
- 新实例必须输出 `dynamicContext`；`foreach/foreachCase` 记录 `type/parameterId/itemValue/caseId`，`custom` 记录 `type/url/nodeType`；`nodeType` 取值为 `catalog | section | slide`；旧 `foreachContext` 仅作为历史数据读取兼容
- Report DSL 冻结阶段遇到 custom 节点时，通过 runtime infrastructure 的外部内容 gateway 以 POST 调用 `url`；正式 v6 请求体包含 `parameters/templateNode/context`，旧 `nodeType/nodeId/prompt` 协议不再作为实现目标
- custom 目录响应按 `status/dsl/meta.dslType=Catalog` 解析，并用 flow `ReportCatalog` 片段替换当前目录，响应必须包含 `name`
- flow custom 章节响应按 `status/dsl/meta.dslType=Section` 解析，并用 flow `ReportSection` 片段替换当前章节，允许包含旧 `summary` 字段
- paged custom slide 响应按 `status/dsl/meta.dslType=Slide` 解析，并用 `ReportSlide` 片段替换当前 slide；paged slide 内 custom section 推荐返回 `dslType=Components` 并合并到当前 slide 组件集合，也允许返回 `Section` 后转换
- HTTP gateway 使用 v6 请求体；paged custom slide 和 slide 内 section 片段进入正式 DSL 合并链路
- 数据集解析在 application 层完成：`sql` 使用固定 `/rest/dte/v1/onequery/uql/query`，`api` 使用模板声明的 `/rest/...` 地址；两者共享 `retCode/retInfo/data.columns/data.results` 统一包络
- 报告查询固定传入 `context["lineage.tracing.enable"] = true`；成功响应每个字段必须返回非空血缘来源数组
- 查询结果字段标题按首个来源的 `businessName_cn -> businessName -> key` 决定，类型读取列级 `type` 并归一化；完整血缘数组进入 DSL，查询层专属字段不进入 DSL
- `retCode != 0` 作为业务失败降级为空数据集，告警写入日志并汇总到 `TemplateInstance.warnings`；HTTP 错误、超时和非法响应仍失败
- `llm/compose` 当前允许出现在模板中，但若被展示块直接引用则返回明确未支持错误
- `compose` 只表示数据集编排声明，不生成独立参数执行绑定
- 静态模板使用 `dataset.source`，运行态模板实例使用 `dataset.sourceRef`；领域模型统一保存后按边界分别序列化
- 外部统一数据集的开放列类型进入 Report DSL 前需要归一化，例如 `number -> double`
- compiler 只消费强类型数据集结果，不直接访问 HTTP
- `text` block 在实例化阶段必须把模板态 `properties.template` 渲染为实例态 `properties.content`，生成 DSL 时编译为 `TextComponent`
- 普通 `table` block 必须编译为 DSL `TableComponent`，透传 `datasetId`、`properties.columns`、`properties.mergeColumns` 与可计算的 `properties.mergeRows`
- `chart` block 必须编译为 DSL `ChartComponent`，透传 `datasetId`
- `properties.preferredType`、普通表格 `properties.columns/showTitle/defaultDisplayRows`、复合表 `tableLayout.columns/showTitle/defaultDisplayRows` 已进入模板定义和后端核心模型；运行时 DSL 编译透传不在本轮实现范围，后续实现阶段再补齐
- `composite_table` 是正式模板块类型，必须被编译为 DSL `compositeTable`
- `composite_table` 的 `query part` 必须把 `part.tableLayout.mergeColumns` 透传到对应子表 `dataProperties.mergeColumns`
- 表格 `dataProperties.data` 已存在时，运行时必须按 `mergeRows` 定义计算 DSL `dataProperties.mergeRows[]`，字段为 `startRowIndex/rowSpan/mergedText/column`
- `summary part` 当前固定编译为无表头静态两列表，运行时只填每行内容
- `TemplateInstance.section.content` 不是派生缓存；实例化阶段就必须落库，供二次编辑和重新生成复用
- `query part` 最小运行态固定为 `status/resolvedDatasetId/resolvedQuery/warnings`
- `summary part` 最小运行态固定为 `status/resolvedPartIds/prompt/warnings`

## 6. 文档导出规则

- 只接受 `Report DSL`
- `word/ppt` 由 Java 导出器生成
- `pdf` 派生转换尚未实现；当前请求返回明确校验错误
- 产物和任务状态分别落 `tbl_report_documents`、`tbl_export_jobs`

## 7. 对外接口映射

- `GET /reports/{reportId}`
- `POST /reports/{reportId}/document-generations`
- `GET /reports/{reportId}/documents/{documentId}/download`

`GET /reports/{reportId}` 返回的 `answer` 结构必须与 `/chat` 里 `REPORT.answer` 完全等价。

## 8. 与流式 `delta` 的边界

- `report` context 只负责产出完整 `Report DSL`
- 目录与章节的流式 `delta` 属于 `conversation` 的对外投影，不写回 `ReportInstance`
- `ReportInstance` 与 `/reports/{reportId}` 始终只返回完整报告，不回放生成阶段的 patch 历史
