...
- 对话通道可以订阅这些事件，转换成 `/chat` JSON 或 SSE 响应；同时可以用当前轮 `chatId` 请求流程协作式停止。
...
flowchart LR
    ...
    conversation -->|"启动 Flow / 订阅事件 / stop(chatId)"| agentflow
...
- 对外停止使用当前轮 `chatId`：`POST /chat/{chatId}/stop`。
...
sequenceDiagram
    ...
    C->>R: start(graph, state={conversationId, chatId, userId})
    R->>Q: status(running)
...
- runtime 内部维护 `chatId -> runId` 索引，供 `/chat/{chatId}/stop` 使用。
...
```mermaid
flowchart TD
    parent["父流程节点"] --> call["call_subflow(alias=analysis_a)"]
    call --> child["子流程<br/>plan -> query -> summarize"]
    child -->|"step: analysis_a.query"| parentEvents["父流程事件队列"]
    child -->|"delta + sourceSubflow"| parentEvents
    child -->|"answer -> state 或 delta"| parentEvents
    parentEvents --> chat["conversation SSE"]
```
