...
- 对外停止使用当前轮 `chatId`：`POST /chat/{chatId}/stop`；`chatId -> runId` 映射归 `conversation` 拥有，不进入 AgentFlow。
...
- runtime 只按 `runId` 提供查询、事件迭代、取消和输入能力；面向 `/chat/{chatId}/stop` 的业务索引由 `conversation` 维护。
