...
`shared/agentflow` 是 ReportSystem 后端公共基础模块，不属于 `conversation`、`report` 或未来 `data_analysis` 等业务 Context。它负责 Flow 运行与 Flow 原始事件预处理，不是最终消息分发中心。

- 业务流程运行时，产生并规范化 `step`、`delta`、`answer`、`error`、`refusal`、`checkpoint` 等原始事件。
- 规范化事件统一发布到 `shared/messaging`；对话、审计、指标等关注方分别注册消费者。
...
flowchart LR
    ...
    agentflow["shared/agentflow<br/>图运行、Flow 事件预处理、工具、子流程"]
    messaging["shared/messaging<br/>统一消息管道与消费者中心"]
    ...
    agentflow -->|"interaction / observability"| messaging
    messaging -->|"interaction consumer"| conversation
```
...
- `MessageCenter`：统一消息中心，接收 `observability.metrics.recorded`，由独立消费者决定如何汇总或外发。
...
- 指标消费者失败不会改变业务流程结果。
- AgentFlow 不绑定 Kafka；后续需要 Kafka、HTTP 或数据库通道时，只向 MessageCenter 注册新的 observability 消费者。
