...
`modules/mock-server` 是独立、可插拔的开发辅助模块。它模拟生产外部业务服务与 OpenAI Compatible 服务，用于联调、演示和自动化测试，不属于 ReportSystem 业务规格，也不挂载到主应用。

- 默认监听 `127.0.0.1:8310`。
- fixture 位于 `testdata/mock-server/`。
- `X-Mock-Scenario: empty | business-error | timeout | error` 可按单次请求模拟空数据、业务失败、超时和 HTTP 错误。
