...
- 用户层：`outline.requirement + outline.items[]`
- 系统层：`content.datasets + presentation`

这意味着：

- 用户在模板工作台和大纲确认里主要面向诉求操作
- 系统在实例生成时主要面向执行链路运行
- 两层在章节节点内显式对应，不是互相替代
- 模板支持“导入预解析 -> 编辑确认 -> 手动保存”流程，导出文件名使用 `模板名称-导出时间.json`
...
- `conversation` 的会话摘要、上下文快照、参数追问、回复生成与 fork helper 也已下沉到上下文本地基础设施模块，根级 `chat_*_service.py / context_state_service.py / param_dialog_service.py` 已进一步清理
- `report_runtime` 上下文当前已完全承接实例创建、定时执行创建、章节重生成与文档技术适配；旧 `application/domain/infrastructure/reporting` 双轨桥接层已移除
- `report_runtime` 的确认大纲、执行基线、文档生成、章节生成 helper 与内容生成器均已迁入上下文本地模块；根级 `outline_review_service.py / template_v2_renderer.py / template_instance_service.py / document_service.py / report_generation_service.py` 已清理
- `conversation` 的能力路由逻辑也已落到上下文本地基础设施模块，根级 `chat_capability_service.py` 已清理
- `template_catalog` 的模板 schema 校验与语义索引逻辑也已下沉到上下文本地基础设施模块，根级 `template_*_service.py` 已清理
