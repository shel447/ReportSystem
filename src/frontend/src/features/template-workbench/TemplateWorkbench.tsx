...
export function TemplateWorkbench({
  ...
  useEffect(() => {
    if (
      !selectedParamKey ||
      !value.parameters.some((item) => item.uiKey === selectedParamKey)
    ) {
  ...
  useEffect(() => {
    if (
      !selectedSectionKey ||
      !findSection(value.sections, selectedSectionKey)
    ) {
      setSelectedSectionKey(value.sections[0]?.uiKey ?? "");
    }
  }, [selectedSectionKey, value.sections]);
  const selectedParameter =
    value.parameters.find((item) => item.uiKey === selectedParamKey) ?? null;
  const selectedSection = useMemo(
    () => findSection(value.sections, selectedSectionKey),
    [selectedSectionKey, value.sections],
  );
  const selectedDataset =
    selectedSection?.content?.datasets.find(
      (item) => item.uiKey === selectedDatasetKey,
    ) ?? null;
  const selectedCompositeSection =
  ...
  const exportJson = useMemo(
    () => prettyJson(toTemplatePayload(value)),
    [value],
  );
  const foreachDisabled = selectedSection
    ? hasForeachAncestor(value.sections, selectedSection.uiKey)
    : false;
  ...
  useEffect(() => {
    ...
    if (
      !selectedDatasetKey ||
      !datasets.some((item) => item.uiKey === selectedDatasetKey)
    ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      !items.some((item) => item.uiKey === selectedCompositeKey)
    ) {
  ...
  function updateStateWithResult(
    mutator: (draft: TemplateWorkbenchState) => boolean,
  ) {
    let changed = false;
    updateState((draft) => {
  ...
  function updateParameter(
    ...
    updateState((draft) => {
      const current = draft.parameters.find(
        (item) => item.uiKey === selectedParameter.uiKey,
      );
      if (!current) {
        return;
      }
      ...
      if (field === "id") {
        ...
        if (
          ...
          draft.previewSamples[previousId] !== undefined
        ) {
  ...
  function addParameter() {
    const item: WorkbenchParameter = {
      ...
      id: suggestId(
        value.parameters.map((entry) => entry.id),
        "param",
      ),
      label: "新参数",
      required: true,
  ...
  function cloneParameter() {
    ...
    const item = {
      ...
      id: suggestId(
        value.parameters.map((entry) => entry.id),
        selectedParameter.id || "param",
      ),
      label: selectedParameter.label
        ? `${selectedParameter.label} 副本`
        : "新参数副本",
    };
    updateState((draft) => {
      const index = draft.parameters.findIndex(
        (entry) => entry.uiKey === selectedParameter.uiKey,
      );
  ...
  function updateSelectedDataset(mutator: (dataset: WorkbenchDataset) => void) {
    ...
    updateSelectedSection((section) => {
      const current = section.content?.datasets.find(
        (item) => item.uiKey === selectedDataset.uiKey,
      );
      ...
      if (previousId && previousId !== current.id) {
        section.content.datasets.forEach((dataset) => {
          dataset.dependsOn = dataset.dependsOn.map((dependency) =>
            dependency === previousId ? current.id : dependency,
          );
          dataset.source.contextRefs = dataset.source.contextRefs.map(
            (dependency) =>
              dependency === previousId ? current.id : dependency,
          );
        });
  ...
  function updateSelectedComposite(
    mutator: (section: WorkbenchCompositeSection) => void,
  ) {
    if (!selectedSection?.content || !selectedCompositeSection) {
      return;
    }
    updateSelectedSection((section) => {
      const current = section.content?.presentation.sections.find(
        (item) => item.uiKey === selectedCompositeSection.uiKey,
      );
      if (current) {
        mutator(current);
      }
  ...
  function updatePresentation(
    mutator: (
      presentation: NonNullable<WorkbenchSection["content"]>["presentation"],
    ) => void,
  ) {
    updateSelectedSection((section) => {
      if (section.content) {
  ...
  function duplicateDataset() {
    ...
    const item = cloneDataset(
      selectedDataset,
      selectedSection?.content?.datasets ?? [],
    );
    updateSelectedSection((section) => {
      if (!section.content) {
        return;
      }
      const index = section.content.datasets.findIndex(
        (entry) => entry.uiKey === selectedDataset.uiKey,
      );
  ...
  function deleteDataset() {
    ...
    updateSelectedSection((section) => {
      ...
      section.content.datasets = section.content.datasets.filter(
        (item) => item.uiKey !== selectedDataset.uiKey,
      );
      section.content.datasets.forEach((dataset) => {
        dataset.dependsOn = dataset.dependsOn.filter(
          (dependency) => dependency !== selectedDataset.id,
        );
        dataset.source.contextRefs = dataset.source.contextRefs.filter(
          (dependency) => dependency !== selectedDataset.id,
        );
      });
  ...
  function indentSection() {
    ...
    const changed = updateStateWithResult((draft) =>
      indentSectionByKey(draft.sections, selectedSection.uiKey),
    );
    if (!changed) {
  ...
  function outdentSection() {
    ...
    const changed = updateStateWithResult((draft) =>
      outdentSectionByKey(draft.sections, selectedSection.uiKey),
    );
    if (!changed) {
      ...
      <div className="template-workbench__grid">
        ...
        <SurfaceCard className="template-workbench__parameters">
          ...
          <div className="list-header">
            ...
            <div>
              {" "}
              <p className="section-kicker">Parameters</p> <h3>参数工作台</h3>{" "}
                ...
                <div className="form-grid form-grid--dense">
                  ...
                  <label className="field">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateParameter("label", event.target.value)
                      }
                    />{" "}
                  </label>{" "}
                  <label className="field">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateParameter("id", event.target.value)
                      }
                    />{" "}
                  </label>{" "}
                  <label className="field">
                    ...
                    <select
                      value={selectedParameter.inputType}
                      onChange={(event) =>
                        updateParameter(
                          "inputType",
                          event.target.value as ParameterInputType,
                        )
                      }
                    >
                      {" "}
                  ...
                  <label className="field">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        updateParameter(
                          "interactionMode",
                          event.target.value as "form" | "chat",
                        )
                      }
                    >
                      {" "}
                  ...
                  <label className="field field--checkbox">
                    ...
                    <label className="choice-chip active">
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateParameter("required", event.target.checked)
                        }
                      />{" "}
                      <span>必填</span>{" "}
                    </label>{" "}
                  ...
                  {selectedParameter.inputType === "dynamic" && (
                    <>
                      ...
                      <label className="field field--full">
                        ...
                        <input
                          value={selectedParameter.source}
                          onChange={(event) =>
                      ...
                      <div className="field field--full">
                        ...
                        <div className="workbench-inline-header">
                          ...
                          <button
                            ...
                            disabled={
                              !selectedParameter.source.trim() ||
                              dynamicOptionsMutation.isPending
                            }
                          >
                            {" "}
                            {dynamicOptionsMutation.isPending
                        ...
                        {dynamicOptionsMutation.error ? (
                          <p className="muted-text">
                            解析失败：
                            {dynamicOptionsMutation.error instanceof Error
                              ? dynamicOptionsMutation.error.message
                              : "请求失败"}
                          </p>
                        ) : null}{" "}
                        {dynamicOptionsMutation.data ? (
                          <div className="inline-panel">
                            ...
                            <p className="muted-text">
                              {" "}
                              返回 {
                                dynamicOptionsMutation.data.meta.returned
                              }{" "}
                              项{" "}
                              {dynamicOptionsMutation.data.meta.retryable
                                ? "，当前错误可重试。"
                                : ""}{" "}
                            </p>{" "}
                          </div>
                  ...
                  <label className="field field--full">
                    ...
                    <input
                      ...
                      value={formatPreviewSample(
                        value.previewSamples[selectedParameter.id],
                      )}
                      placeholder={
                        selectedParameter.multi
                          ? "使用逗号分隔多项"
                          : "输入预览值"
                      }
                      onChange={(event) =>
        ...
        <SurfaceCard className="template-workbench__sections">
          ...
          <div className="list-header">
            ...
            <div>
              {" "}
              <p className="section-kicker">Sections</p> <h3>章节工作台</h3>{" "}
              <p className="muted-text">
                维护章节树、foreach 和内容展示方式。
              </p>{" "}
            </div>{" "}
            <div className="action-row action-row--compact">
              {" "}
              ...
              {selectedSection ? (
                <div className="template-workbench__detail">
                  ...
                  {selectedSectionTab === "outline" ? (
                    <div
                      className="template-workbench__detail"
                      role="tabpanel"
                      aria-label="诉求"
                    >
                      {" "}
                      <div className="form-grid form-grid--dense">
                        ...
                        <label className="field">
                          ...
                          <select
                            ...
                            onChange={(event) =>
                              updateSelectedSection((section) => {
                                section.kind = event.target
                                  .value as WorkbenchSection["kind"];
                      ...
                      <div className="inline-panel">
                        ...
                        <label className="field field--checkbox">
                          ...
                          <label className="choice-chip active">
                            ...
                            <input
                              type="checkbox"
                              checked={selectedSection.foreachEnabled}
                              onChange={(event) =>
                                updateSelectedSection((section) => {
                                  section.foreachEnabled = event.target.checked;
                                  if (!event.target.checked) {
                                    section.foreachParam = "";
                                    section.foreachAlias = "item";
                                  }
                                })
                              }
                            />{" "}
                        ...
                        {selectedSection.foreachEnabled ? (
                          <div className="form-grid form-grid--dense">
                            ...
                            <label className="field">
                              ...
                              <select
                                value={selectedSection.foreachParam}
                                onChange={(event) =>
                                  updateSelectedSection((section) => {
                                    section.foreachParam = event.target.value;
                                  })
                                }
                              >
                                {" "}
                                ...
                                {value.parameters
                                  ...
                                  .map((item) => (
                                    <option key={item.uiKey} value={item.id}>
                                      {item.label || item.id}
                                    </option>
                                  ))}{" "}
                              </select>{" "}
                            ...
                            <label className="field">
                              ...
                              <input
                                value={selectedSection.foreachAlias}
                                onChange={(event) =>
                                  updateSelectedSection((section) => {
                                    section.foreachAlias = event.target.value;
                                  })
                                }
                              />{" "}
                            </label>{" "}
                      ...
                      {selectedSection.outline ? (
                        <div className="editor-block requirement-editor-shell">
                          {" "}
                          <div className="requirement-editor-main">
                            <div className="list-header">
                              {" "}
                              <div>
                                {" "}
                            ...
                            <div className="form-grid form-grid--dense">
                              ...
                              <label className="field field--full">
                                ...
                                <textarea
                                  aria-label="诉求文稿"
                                  rows={4}
                                  value={selectedSection.outline.requirement}
                                  onChange={(event) =>
                                    updateSelectedSection((section) => {
                                      if (section.outline) {
                            ...
                            <div className="requirement-item-list">
                              ...
                              <div className="requirement-item-list__stack">
                                {selectedSection.outline.items.length ? (
                                  ...
                                  <EmptyState
                                    title="暂无诉求要素"
                                    description="添加要素后，可在右侧检查器里继续编辑细节。"
                                  />
                                )}
                              </div>
                            </div>
                          </div>
                          <aside className="requirement-editor-inspector">
                            <div className="inline-panel">
                              <strong>诉求检查器</strong>
                              <p>
                                当前句子中的可替换部分在这里维护。短字段保持紧凑，
                                只在需要时展开详细配置。
                              </p>
                            </div>
                        ...
                        <div className="inline-panel">
                          ...
                          <button
                            ...
                            onClick={() =>
                              updateSelectedSection((section) => {
                                section.outline = createRequirementOutline();
                              })
                            }
                          >
                            启用诉求
                            ...
                            <div className="workbench-split">
                              ...
                              <div className="workbench-pane">
                                ...
                                {selectedSection.content.datasets.length ? (
                                  <div className="list-stack">
                                    {" "}
                                    {selectedSection.content.datasets.map(
                                      (dataset) => (
                                        <button
                                          key={dataset.uiKey}
                                          type="button"
                                          ...
                                          onClick={() =>
                                            setSelectedDatasetKey(dataset.uiKey)
                                          }
                                        >
                                          {" "}
                                          <strong>
                                ...
                                {selectedDataset ? (
                                  <div className="form-grid form-grid--dense">
                                    ...
                                    <label className="field">
                                      ...
                                      <select
                                        aria-label="数据源类型"
                                        value={selectedDataset.source.kind}
                                        onChange={(event) =>
                                          updateSelectedDataset((dataset) => {
                                            dataset.source.kind = event.target
                                              .value as DatasetSourceKind;
                                          })
                                        }
                                      >
                                        {" "}
                                      ...
                                      <div className="chip-grid">
                                        ...
                                        {selectedSection.content.datasets
                                          ...
                                          .map((item) => (
                                            <label
                                              key={item.uiKey}
                                              className={`choice-chip ${selectedDataset.dependsOn.includes(item.id) ? "active" : ""}`}
                                            >
                                              {" "}
                                              <input
                                                type="checkbox"
                                                checked={selectedDataset.dependsOn.includes(
                                                  item.id,
                                                )}
                                                onChange={(event) =>
                                    ...
                                    <label className="field field--full">
                                      ...
                                      <input
                                        value={
                                          selectedDataset.source.description
                                        }
                                        onChange={(event) =>
                                          updateSelectedDataset((dataset) => {
                                    ...
                                    {selectedDataset.source.kind === "sql" ? (
                                      <label className="field field--full">
                                        ...
                                        <textarea
                                          aria-label="SQL 文本"
                                          rows={6}
                                          value={selectedDataset.source.query}
                                          onChange={(event) =>
                                            updateSelectedDataset((dataset) => {
                                    ...
                                    "ai_synthesis" ? (
                                      <>
                                        ...
                                        <label className="field field--full">
                                          ...
                                          <input
                                            value={selectedDataset.source.contextRefs.join(
                                              ", ",
                                            )}
                                            onChange={(event) =>
                                              updateSelectedDataset(
                                                (dataset) => {
                                                  dataset.source.contextRefs =
                                                    splitCsv(
                                                      event.target.value,
                                                    );
                                                },
                                        ...
                                        <label className="field field--full">
                                          ...
                                          <textarea
                                            rows={4}
                                            value={selectedDataset.source.contextQueries
                                              .map(
                                                (item) =>
                                                  `${item.id}: ${item.query}`,
                                              )
                                        ...
                                        <label className="field field--full">
                                          ...
                                          <textarea
                                            ...
                                            onChange={(event) =>
                                              updateSelectedDataset(
                                                (dataset) => {
                                                  dataset.source.knowledgeQueryTemplate =
                                                    event.target.value;
                                                },
                                              )
                                            }
                                        ...
                                        <div className="form-grid form-grid--dense">
                                          ...
                                          <label className="field">
                                            ...
                                            <input
                                              value={
                                                selectedDataset.source
                                                  .knowledgeParams.subject
                                              }
                                              onChange={(event) =>
                                                updateSelectedDataset(
                                                  (dataset) => {
                                                    dataset.source.knowledgeParams.subject =
                                                      event.target.value;
                                                  },
                                                )
                                              }
                                          ...
                                          <label className="field">
                                            ...
                                            <input
                                              value={
                                                selectedDataset.source
                                                  .knowledgeParams.symptoms
                                              }
                                              onChange={(event) =>
                                                updateSelectedDataset(
                                                  (dataset) => {
                                                    dataset.source.knowledgeParams.symptoms =
                                                      event.target.value;
                                                  },
                                                )
                                              }
                                          ...
                                          <label className="field">
                                            ...
                                            <input
                                              value={
                                                selectedDataset.source
                                                  .knowledgeParams.objective
                                              }
                                              onChange={(event) =>
                                                updateSelectedDataset(
                                                  (dataset) => {
                                                    dataset.source.knowledgeParams.objective =
                                                      event.target.value;
                                                  },
                                                )
                                              }
                            ...
                            <div className="form-grid form-grid--dense">
                              ...
                              <label className="field">
                                ...
                                <select
                                  aria-label="展示类型"
                                  value={
                                    selectedSection.content.presentation.type
                                  }
                                  onChange={(event) =>
                                    updatePresentation((presentation) => {
                                      presentation.type = event.target
                                        .value as typeof presentation.type;
                                    })
                                  }
                                >
                                  {" "}
                              ...
                              selectedSection.content.presentation.type ===
                                ...
                                <label className="field">
                                  ...
                                  <select
                                    aria-label="绑定数据集"
                                    value={
                                      selectedSection.content.presentation
                                        .datasetId
                                    }
                                    onChange={(event) =>
                                      updatePresentation((presentation) => {
                                        presentation.datasetId =
                                          event.target.value;
                                      })
                                    }
                                  >
                              ...
                              "composite_table" ? (
                                <>
                                  ...
                                  <FieldArrayEditor
                                    ...
                                    onRemove={(index) =>
                                      updatePresentation((presentation) => {
                                        presentation.sections.splice(index, 1);
                                      })
                                    }
                  ...
                  {selectedSectionTab === "sync" ? (
                    <div
                      className="inline-panel sync-panel"
                      role="tabpanel"
                      aria-label="同步状态"
                    >
                      {" "}
                      <div className="sync-panel__header">
                        {" "}
                        <span
                          className={`status-chip status-chip--${selectedOutlineSummary?.status ?? "not_configured"}`}
                        >
                          {
                            OUTLINE_SYNC_STATUS_LABELS[
                              selectedOutlineSummary?.status ?? "not_configured"
                            ]
                          }
                        </span>{" "}
                        <span className="muted-text">
                          {" "}
                          {selectedOutlineSummary?.status === "in_sync"
                            ? `执行链路已引用 ${selectedOutlineSummary.bindings.length} 个诉求要素。`
                            : null}{" "}
                          {selectedOutlineSummary?.status === "stale"
                            ? "已配置诉求，但执行链路还没有使用任何诉求要素引用。"
                            : null}{" "}
                          {selectedOutlineSummary?.status === "not_configured"
        ...
        <SurfaceCard className="template-workbench__preview">
          ...
          <div className="list-header">
            ...
            <div>
              {" "}
              <p className="section-kicker">Preview</p> <h3>结构预览</h3>{" "}
              <p className="muted-text">
                切换查看诉求层、执行层和模板 JSON。
              </p>{" "}
            </div>{" "}
...
function SectionTreeItem({
  ...
  return (
    <div className="section-tree-item" style={{ paddingLeft: depth * 14 }}>
      ...
      <button
        type="button"
        className={`list-item ${section.uiKey === selectedKey ? "active" : ""}`}
        onClick={() => onSelect(section.uiKey)}
      >
        {" "}
        <strong>{section.title || "未命名章节"}</strong>{" "}
        <span>
          {[
            section.foreachEnabled
              ? `foreach: ${section.foreachParam || "未设置"}`
              : null,
            section.kind === "group"
      ...
      {section.children.length ? (
        <div className="section-tree-item__children">
            ...
            <label className="field">
              ...
              >
                ...
                {parameters.map((parameter) => (
                  <option key={parameter.uiKey} value={parameter.id}>
                    {parameter.label || parameter.id}
                  </option>
                ))}{" "}
              </select>{" "}
            </label>{" "}
            <p className="outline-block-editor__hint">
              该要素的取值直接来自全局参数，不在章节诉求内重复配置选项。
            </p>{" "}
          </>
        ) : null}{" "}
...
function normalizeOperatorValue(value: string) {
  return OPERATOR_OPTIONS.includes(value as (typeof OPERATOR_OPTIONS)[number])
    ? value
    : ">=";
}
...
function createCompositeSection(
  existing: WorkbenchCompositeSection[],
): WorkbenchCompositeSection {
  return {
...
function removeSection(
  ...
  return sections
    .filter((section) => section.uiKey !== uiKey)
    .map((section) => ({
      ...section,
      children: removeSection(section.children, uiKey),
    }));
}
function insertSiblingAfter(
  ...
  return sections.some((section) =>
    insertSiblingAfter(section.children, targetKey, nextSection),
  );
}
function moveSectionByKey(
  sections: WorkbenchSection[],
  targetKey: string,
  delta: number,
): boolean {
  ...
  return sections.some((section) =>
    moveSectionByKey(section.children, targetKey, delta),
  );
}
function hasForeachAncestor(
  sections: WorkbenchSection[],
  uiKey: string,
  ancestorForeach = false,
): boolean {
  for (const section of sections) {
    if (section.uiKey === uiKey) {
    ...
    if (
      hasForeachAncestor(
        section.children,
        uiKey,
        ancestorForeach || section.foreachEnabled,
      )
    ) {
      return true;
    }
...
function indentSectionByKey(
  ...
  return sections.some((section) =>
    indentSectionByKey(section.children, targetKey),
  );
}
function outdentSectionByKey(
  sections: WorkbenchSection[],
  targetKey: string,
): boolean {
...
function outdentWithin(
  ...
  for (
    ...
    if (
      outdentWithin(
        sections[currentIndex].children,
        targetKey,
        sections,
        currentIndex,
      )
...
function moveArrayItemByKey<T extends { uiKey: string }>(
  items: T[],
  uiKey: string,
  delta: number,
) {
...
function buildSelectedParams(
  ...
  return Object.fromEntries(
    state.parameters
      ...
      .map((parameter) => [
        parameter.id,
        state.previewSamples[parameter.id] ?? "",
      ]),
  );
}
