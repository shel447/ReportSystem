    ...
    *,
    ...
    if matched_template:
        user_lines.extend([
            ...
            f"- 场景: {matched_template.get('scenario') or '未设置'}",
    ...
    if candidates:
        ...
        for item in candidates:
            user_lines.append(
                f"- {item.get('template_name')} | 场景: {item.get('scenario') or '未设置'} | 描述: {item.get('description') or '未设置'} | 类型: {item.get('report_type') or '未设置'} | 评分: {item.get('score')} | 原因: {', '.join(item.get('match_reasons') or [])}"
            )
