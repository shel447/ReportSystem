...
from backend.infrastructure.persistence.dev_database import DevBase
from backend.infrastructure.persistence import dev_models, models  # noqa: F401
from backend.infrastructure.persistence.upgrades import DatabaseUpgradeError, apply_upgrades


PERSISTENCE_DIR = Path(__file__).resolve().parents[1] / "infrastructure" / "persistence"
...
def test_business_upgrades_create_current_schema_and_are_idempotent(tmp_path):
    ...
    assert apply_upgrades(database_path=db_path, upgrades_dir=BUSINESS_UPGRADES, engine=engine, metadata=Base.metadata) == 1
    assert apply_upgrades(database_path=db_path, upgrades_dir=BUSINESS_UPGRADES, engine=engine, metadata=Base.metadata) == 1
    ...
    with sqlite3.connect(db_path) as connection:
        assert connection.execute("SELECT current_version FROM __db_schema_version WHERE id = 1").fetchone() == (1,)
