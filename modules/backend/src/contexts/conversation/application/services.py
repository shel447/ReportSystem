...
from ...report.application.generation_models import DocumentView, GenerationProgressView, ReportAnswerView
from ...report.domain.generation_models import ReportInstance, TemplateInstance, report_dsl_from_dict, template_instance_from_dict
from ...report.domain.generation_services import (
...
from ...report.application.template_models import TemplateImportPreview
from ...report.domain.template_models import (
    Parameter,
    ParameterValue,
    ReportTemplate,
    parameter_from_dict,
...
class ConversationService:
    ...
    def _extract_report_template(self, *, data: ChatCommand, user_id: str) -> ChatResponse:
        """将自由文本输入投影为模板导入预览结果。"""
        normalized = self.template_management_service.preview_import_template(data.question or {})
        return ChatResponse(
    ...
    def _generate_report(self, *, data: ChatCommand, user_id: str) -> ChatResponse:
        ...
        if reply and str(reply.type or "") == "confirm_params":
            ...
            template = self.template_management_service.get_template(template_id)
