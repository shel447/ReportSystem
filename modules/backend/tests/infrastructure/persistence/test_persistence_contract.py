...
def test_business_upgrades_create_current_schema_and_are_idempotent(tmp_path):
    ...
    assert apply_upgrades(database_path=db_path, upgrades_dir=BUSINESS_UPGRADES, engine=engine, metadata=Base.metadata) == 4
    assert apply_upgrades(database_path=db_path, upgrades_dir=BUSINESS_UPGRADES, engine=engine, metadata=Base.metadata) == 4
...
def test_v004_removes_local_user_mirror_without_losing_owned_resources(tmp_path):
    ...
    engine = _engine(db_path)
    assert apply_upgrades(database_path=db_path, upgrades_dir=legacy_upgrades, engine=engine, metadata=Base.metadata) == 3
    with sqlite3.connect(db_path) as connection:
