...
def test_parameter_dataset_and_dynamic_protocols():
    ...
    assert client.post("/rest/onequery", json={"query": "select * from network_health", "context": {}}).json()["data"]["results"][0]["results"]
    assert client.post("/rest/datasets/availability-trend", json={"parameters": {}, "context": {}}).json()["data"]["results"][0]["results"]
...
def test_header_scenarios_are_request_scoped():
    assert client.post("/rest/datasets/availability-trend", headers={"X-Mock-Scenario": "empty"}, json={}).json()["data"]["results"][0]["results"] == []
    ...
    assert client.post("/rest/datasets/availability-trend", json={}).json()["data"]["results"][0]["results"]
